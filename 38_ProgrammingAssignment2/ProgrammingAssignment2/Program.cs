    ...
    class Program
    {
		...
		static void Main(string[] args)
		{
			...
			while (input[0] != 'q')
			{
                ...
                int countOfNumbers = values.Count;
                int sumOfNumbers = 0;
				float meanOfNumbers = 0;
                while (lastValue != -1)
				{
                    sumOfNumbers += lastValue;
                    lastValue = GetValue();
					
                }

                meanOfNumbers = (float)sumOfNumbers / countOfNumbers;
                if (countOfNumbers == 0)
                {
                    meanOfNumbers = 0;
                }
                
                Console.WriteLine(countOfNumbers + " " + meanOfNumbers);
